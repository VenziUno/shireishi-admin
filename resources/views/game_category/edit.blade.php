...
<script>
    $('#form-edit').submit(function(e) {
        e.preventDefault();
        $.ajax({
            url:"{{route('game-category_edit_patch', $category->id)}}",
            type:"PATCH",
            data: $('#form-edit').serialize(),
            success:function(res) {
                // console.log(res);
                toastr['success']("Kategori Permainan berhasil diesit");
                window.location.href = "{{route('game-category_view_index')}}";
            },
            error:function(res) {
                if (res.status != 422)
