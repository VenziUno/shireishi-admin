...
<script>
    $('#form-edit').submit(function(e) {
        e.preventDefault();
        $.ajax({
            url:"{{route('game-news-category_edit_patch', $category->id)}}",
            type:"PATCH",
            data: $('#form-edit').serialize(),
            success:function(res) {
                // console.log(res);
                toastr['success']("Kategori Berita Permainan berhasil diedit");
                window.location.href = "{{route('game-news-category_view_index')}}";
            },
            error:function(res) {
                if (res.status != 422)
