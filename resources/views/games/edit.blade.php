            ...
            <h4><b><u>Detail Permainan</u></b></h4>
                ...
                <div class="row mb-3">
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            @foreach($game->HasCategory as $arr)
                                <input type="hidden" value="{{$arr->game_categories_id}}" name="has[]">
                            @endforeach
                        ...
                        value="{{ $game->name }}" type="text" />

                        <div class="form-group">
                            <label for="content">Deskripsi</label>
                            <textarea name="desc" id="desc" class="form-control" cols="30" rows="10"> {!! $game->description !!} </textarea>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="redirect_link" id="redirect_link" class="form-control" value="{{ $game->	redirect_link }}">
                        ...
                        <x-input-text name="order" id="order" label="Urutan"
                            value="{{ $game->order }}" type="text" />
    
                        <div class="form-group">
                            <label class="control-label" for="color">Warna Latar Belakang</label>
                            <input type="text" name="color" id="color" class="form-control" value="{{ $game->color_background }}">
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="hidden" name="icon" id="icon" value="{{$game->small_icon}}">
                            </div>
                            <br>
                            @if(strpos($game->small_icon, 'jpg') == TRUE || strpos($game->small_icon, 'jpeg') == TRUE || strpos($game->small_icon, 'png') == TRUE || strpos($game->small_icon, 'JPG') == TRUE || strpos($game->small_icon, 'JPEG') == TRUE || strpos($game->small_icon, 'PNG') == TRUE)
                                <img src="{{\App\Helper\Helper::serveImage($game->small_icon)}}" id="photoNameIcon" style="width:300px;">'
                            @endif
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="hidden" name="picture" id="picture" value="{{$game->cover_image}}">
                            </div>
                            <br>
                            @if(strpos($game->cover_image, 'jpg') == TRUE || strpos($game->cover_image, 'jpeg') == TRUE || strpos($game->cover_image, 'png') == TRUE || strpos($game->cover_image, 'JPG') == TRUE || strpos($game->cover_image, 'JPEG') == TRUE || strpos($game->cover_image, 'PNG') == TRUE)
                                <img src="{{\App\Helper\Helper::serveImage($game->cover_image)}}" id="photoNamePicture" style="width:300px;">'
                            @endif
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="hidden" name="web" id="web" value="{{$game->web_background_image}}">
                            </div>
                            <br>
                            @if(strpos($game->web_background_image, 'jpg') == TRUE || strpos($game->web_background_image, 'jpeg') == TRUE || strpos($game->web_background_image, 'png') == TRUE || strpos($game->web_background_image, 'JPG') == TRUE || strpos($game->web_background_image, 'JPEG') == TRUE || strpos($game->web_background_image, 'PNG') == TRUE)
                                <img src="{{\App\Helper\Helper::serveImage($game->web_background_image)}}" id="photoNameWeb" style="width:300px;">'
                            @endif
                ...
                <div class="form-group">
                    ...
                    <div class="photo-content row">
                        @foreach($game->Photo as $p)
                            @if(strpos($p->link, 'jpg') == TRUE || strpos($p->link, 'jpeg') == TRUE || strpos($p->link, 'png') == TRUE || strpos($p->link, 'JPG') == TRUE || strpos($p->link, 'JPEG') == TRUE || strpos($p->link, 'PNG') == TRUE)
                                <div class="content mt-3 col-3 text-center d-flex align-items-center">
                                    <input type="hidden" name="sub_photo[]" id="sub_photo[]" value="{{$p->link}}">    
                                    <img src="{{\App\Helper\Helper::serveImage($p->link)}}" id="photoName" style="width:300px;">
                            ...
                            @else 
                                <div class="content mt-3 col-3 text-center d-flex align-items-center">
                                    ...
                                    <video width="300" controls>
                                        <source src="{{\App\Helper\Helper::staticPath($p->link)}}" type="video/mp4">
                                    </video>
                ...
                <div class="link-content">
                    @if(count($game->DownloadLink) > 0)
                        @foreach($game->DownloadLink as $l)
                            <div class="content row mt-3">
                                <div class="col-sm-5">
                                    <input type="text" class="form-control" name="link_name[]" id="link_name[]" value="{{$l->name}}">
                                </div>
                                <div class="col-sm-6">
                                    <input type="text" class="form-control" name="link[]" id="link[]" value="{{$l->redirect_link}}">
                                </div>
                                <div class="col-sm-1">
...
<script>
    ...
    $(document).ready(function(e) {
        ...
        $('#category').select2();
        $('#category').val(selectedValuesTest).trigger('change');
    })
    ...
    $('#form-edit').submit(function(e) {
        ...
        $.ajax({
            url:"{{route('game_edit_patch', $game->id)}}",
            type:"PATCH",
            data: $('#form-edit').serialize(),
            success:function(res) {
                // console.log(res);
                toastr['success']("Games Berhasil diedit");
